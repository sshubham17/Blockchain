...
// function that take the registration details from the frontend and pass it to the contract
  ...
  async Register(event){
   ...
   try {
    ...
    console.log(propertyId,typeof(this.survey));
   ...
   //  calling the contract function for registration and passing the values including the calculated property ID
    ...
    /*const transaction = await contract.methods.Registration(this.state,this.RegistrationAddress,propertyId).send({
      from : this.account});
    */
    console.log(this.district,typeof(this.district));
     console.log(transaction);
     if(!transaction){
       console.log("Transaction Faild!!!");
     }
       ...
       console.log("Transaction succesful");
       alert("Proprty get Register Succesfully")
     }
     } catch (error) {
        console.log("error:",error);
    }
